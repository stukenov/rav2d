...
//!     `dav2d_dc_ibp_weights`) → `src/ipred_neon.rs`.
//!
...
/// read-only tables they reference (`dav2d_sm_weights`, `dav2d_dc_ibp_weights`),
...
/// `dav2d_sm_weights` / `dav2d_dc_ibp_weights` references resolve.
fn generate_ipred_tables(tables_c: &Path, out: &Path) {
    ...
    // The intra-block-prediction DC weights, referenced by the DC kernels.
    let dc_ibp_weights = grab("dav2d_dc_ibp_weights[32]");
